...
try {
  ...
  const report = buildCredentialRotationActionPack(JSON.parse(source));
  console.log(JSON.stringify({
    incidentPath,
    ...publicCredentialRotationActionPack(report)
  }, null, 2));
  process.exitCode = report.valid ? 0 : 1;
} catch (error) {
  const report = buildCredentialRotationActionPack(null, {
