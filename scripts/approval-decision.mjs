...
const requireApproved = args.includes("--require-approved") || process.env.ALLOW_REQUIRE_APPROVED_DECISION === "1";
const positional = args.filter((arg) => arg !== "--require-approved");
const decisionPath = resolve(positional[0] || process.env.ALLOW_APPROVAL_DECISION_PATH || join(root, "work/approval-decision.template.json"));
const workspaceDir = resolve(positional[1] || process.env.ALLOW_EXTERNAL_ACTION_WORKSPACE_DIR || join(root, "work/external-action-workspace"));
...
try {
  ...
  const validation = validateActionTimeApprovalDecision(decision.value, {
    approvalRequest,
    draftPacket: draftPacket.value,
  ...
  const reasons = [
    ...
    ...draftPacket.reasons,
    ...decision.reasons,
    requireApproved && validation.decision !== "approved"
      ? "Approval decision must be approved when --require-approved is set"
      : null
  ].filter(Boolean);
  const valid = validation.valid && draftPacket.reasons.length === 0 && decision.reasons.length === 0 && (!requireApproved || validation.decision === "approved");
  ...
  console.log(JSON.stringify({
    ...publicActionTimeApprovalDecisionReport({
      ...
      nextAction: valid
        ...
        : "Fix the approval decision, draft packet, or request context before editing the packet."
    }),
    requireApproved,
    decisionPath,
    workspaceDir
  ...
  console.error(`Approval decision validation failed: ${error.message}`);
  process.exit(1);
}
